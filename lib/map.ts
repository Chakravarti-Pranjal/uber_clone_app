...
//             // );
//             const responseToUser = await fetch(
//                 `https://api.openrouteservice.org/v2/directions/driving-car?api_key=${directionsAPI}&start=${marker.longitude},${marker.latitude}&end=${userLongitude},${userLatitude}`,
//             );
  ...
  try {
    const timesPromises = markers.map(async (marker) => {
      try {
        // Driver → User
        const responseToUser = await fetch(
          `https://api.openrouteservice.org/v2/directions/driving-car?api_key=${process.env.EXPO_PUBLIC_ORS_API_KEY}&start=${marker.longitude},${marker.latitude}&end=${userLongitude},${userLatitude}`
        );
        const dataToUser = await responseToUser.json();
        ...
        const dataToDestination = await responseToDestination.json();
        const timeToDestination = dataToDestination?.routes?.[0]?.summary?.duration || 0; // seconds
